...
Page({
  data: {
    ...
    musicIndex: 0,
    musicUrl: [
      'http://www.mayongcheng.cn/static/piano.mp3',
      'http://www.mayongcheng.cn/static/violin.mp3',
      'http://www.mayongcheng.cn/static/erhu.mp3',
    ]
  ...
  draw: function (pre, text, dir) {  //画时钟
    ...
    var grd = ctx.createLinearGradient(0, 0, 100, 0)//(0,0,200,0)
    ...
    ctx.beginPath()
    ctx.arc(center, center, center / 2, -0.5 * Math.PI, -0.5 * Math.PI+pre , false);//顺时针 起始角  结束角 弧度
    //console.log('begin:' + (-0.5 * Math.PI))
    //console.log('over:' +( -0.5 * Math.PI + pre))
    ctx.stroke();

    ctx.setFillStyle('white')//时间颜色
  ...
  drawBack() {  // 画背景
    ...
    ctx.setLineWidth(4)
    ctx.setLineCap('round')
    ctx.setStrokeStyle('rgb(169,169,169)')//没填满的地方
    ctx.beginPath()
    ctx.arc(center, center, center / 2, -0.5 * Math.PI, 1.5 * Math.PI);//1.5
    ctx.stroke();
    ctx.draw()
  },
  ...
  onLoad: function () { //程序加载
    ...
    app.globalData.tomatoNum = tomatoNum == '' ? 0 : tomatoNum // 5:tomatoNum
    app.globalData.tomatoToday = tomatoToday == '' ? new Date(1997, 1, 8) : tomatoToday
    ...
    app.globalData.rest = rest == '' ? 0 : rest
    app.globalData.count = count == '' ? 1 : count//1
    app.globalData.musicIndex = musicIndex == '' ? 0 : musicIndex //1
    ...
    if (app.globalData.rTime <= 0) {
      if (that.data.rest) {
        ...
        if (that.data.count <= 0) {
          ...
          wx.showModal({
            title: '提示',
            content: '完成任务了，得到' + 1 + '个大番茄', // +tomato+
            showCancel: false,
            confirmText: '我知道了',
            success: function (res) {
              app.globalData.stop = false
              wx.navigateTo({
                url: '../tomato/tomato?state=true&data=1',//data = tomato
              })
            }
          })
          return
  ...
  // insertUserFunction(e) {
  // //   var that = this;
  // //   var success = app.globalData.tomatoToday;
  // //   var OPENID = that.data.openid;
    
  // // }
  // if(app.globalData.tomatoToday){
  //  wx.request({
  //    url: 'http://www.mayongcheng.cn/punch/study?openid='+that.data.openid,
  //    method: 'GET',
  //    success:function(res){
